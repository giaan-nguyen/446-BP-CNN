...
# simulation
def simulation_colored_noise(generate_data_for, linear_code, top_config, train_config, net_config, simutimes_range, target_err_bits_num, batch_size):
...
# simutimes_range: [min_simutimes, max_simutimes]
    ...
    if generate_data_for == 'Training':
        ...
        max_batches = int(train_config.training_sample_num // train_config.training_minibatch_size)
    elif generate_data_for == 'Test':
        batch_size = int(train_config.test_minibatch_size // np.size(train_config.SNR_set_gen_data))
        max_batches = int(train_config.test_sample_num // train_config.test_minibatch_size)
    else:
    ...
    ### NEW STUFF HERE ###
    if generate_data_for == 'Training':
        fout_est_noise = open("./TrainingData/EstNoise_after_cnn1_bp.dat", 'wb')
        fout_real_noise = open("./TrainingData/RealNoise_after_cnn1_bp.dat", 'wb')
    elif generate_data_for == 'Test':
        fout_est_noise = open("./TestData/EstNoise_after_cnn1_bp.dat", 'wb')
        fout_real_noise = open("./TestData/RealNoise_after_cnn1_bp.dat", 'wb')
    else:
        print('Invalid objective of data generation!')
        exit(0)
    ...
    for SNR in SNRset:
        ...
        for ik in range(0, max_batches):
            ...
            ### NEW STUFF HERE ###
            noise_after_cnn_bp = y_receive - (u_BP_decoded * (-2) + 1)
            noise_after_cnn_bp = noise_after_cnn_bp.astype(np.float32)
            noise_after_cnn_bp.tofile(fout_est_noise)  # write features to file
            ch_noise.tofile(fout_real_noise)  # write labels to file
